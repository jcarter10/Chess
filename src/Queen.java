/*
 * By: Jordan Carter
 * Description: Contains all of the chess rules for the Queen chess piece.
 * 
 */
...
public class Queen extends ChessPiece {
	...
	public Queen(int row, int col, pType piece, boolean color, Icon ic, int moveCount) {
	...
	public static boolean checkValidity(ChessPiece cp1, ChessPiece cp2, int goRow, int goCol) {
		...
		//queen moving on horizontal line.
		...
		//queen moving on diagonal line.
		else if (Math.abs(cp1.getRow() - goRow) == Math.abs(cp1.getCol() - goCol)) {
			direction = "diagonal";
			validMove = (Math.abs(goRow - cp1.getRow()) > 0 && Math.abs(goRow - cp1.getRow()) < 8) ? true : false;
	...
	private static boolean checkCollisions(ChessPiece cp1, ChessPiece cp2, int goRow, int goCol, String direction) {
		...
		if (direction.equals("vertical") == true) {
			//checks the row above the queen, to location.
			if (cp1.getRow() > goRow) {
			...
			//checks the row under the queen, to location.
			else {
				for (row = cp1.getRow() + 1; row < goRow; row++) {
					foundPiece = ChessBoard.getChessPiece(row, goCol);
					//piece was found in path to location.
		...
		//horizontal check.
		else if (direction.equals("horizontal") == true){
			//checks the col to the left of the queen, to the location.
			if (cp1.getCol() > goCol) {
			...
			//checks the col to the right of the queen, to the location.
			else {
				for (col = cp1.getCol() + 1; col < goCol; col++) {
					foundPiece = ChessBoard.getChessPiece(goRow, col);
					//piece was found in path to location.
		...
		//diagonal check.
		else if (direction.equals("diagonal") == true) {
			//Queen moving North-East.
			if (goRow < cp1.getRow() && goCol > cp1.getCol()) {
				while (i < 8) {
					foundPiece = ChessBoard.getChessPiece(cp1.getRow() - i, cp1.getCol() + i);
					//returns when taken piece location is found.
